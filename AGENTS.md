...
## 2026-08-16 — Launch walk extended to file materials, review queue, saved folders (36/36)
- **Extended `scripts/launch-walk.mjs`** from 20 → 36 checks, covering three more checklist journeys with the same RLS-honest pattern (real user JWTs, no SQL):
  - **File materials (PDF preview + image lightbox)**: mirror `uploadFileMaterial()` — insert type=`file` row → upload to `materials` bucket at `${author_id}/${material_id}/…` (storage own-folder insert policy) → `storage_objects` mirror row → set `storage_path` + `metadata.mime` → signed-URL read-back that round-trips the exact bytes (218-byte PDF / 70-byte PNG, content-type verified) → assert the non-uploader member is blocked by the uploader-only storage select policy (`Object not found`).
  - **Review queue**: mirror `createQuizReviewDeck()` — a second quiz where Bob misses Q2 (attempt 50%), deck inserted as `flashcard_set` material with `metadata {cards, is_quiz_review:true, quiz_id}`, card front/back built from the missed question, and the exact `getQuizReviewDeck()` metadata-query re-finds the same deck (idempotent → “Review deck” button state).
  - **Saved folders**: mirror the /app/saved flow — `saved_collections` folder insert → `saved_items` upserts for a thread + material (polymorphic item_type) → folder read-back lists both → deleting the folder returns items to Uncategorized via the FK `on delete set null`.
- **Cleanup hardened**: storage files removed via service key (uploader-only storage RLS would block user-JWT deletes), plus `storage_objects`, `saved_items`, `saved_collections` rows. Verified zero leftovers after the run: 0 walk users/spaces/storage rows/saved items in live, and the materials bucket holds only the pre-existing `test.txt`.
- **Result: 36/36 checks pass, exit 0** against live.
...
## 2026-08-15 — Gate now runs lint/tsc/test too
- **`scripts/pre-deploy-gate.mjs`**: added the code-quality tier before the config checks — lint → tsc → test → env → migrations → push → digest (fail-fast). Steps run via `npm run <id> --silent` (new `"tsc": "tsc --noEmit"` script added to package.json); `runStep` picks npm vs direct-node per step. `--skip` ids now include lint,tsc,test. Local run (skip env,push,digest): lint ✓ tsc ✓ test ✓ migrations ✓ → exit 0. Docs: LAUNCH_CHECKLIST (gate is now the single pre-deploy command), DEPLOYMENT (CI gate section updated).

## 2026-08-15 — CI pre-deploy gate workflow
- **`.github/workflows/pre-deploy-gate.yml`** (new): runs on every push/PR — `npm ci` → `gate:predeploy` → `npm run build` (build only runs when the gate passes). Steps whose repo secrets aren't configured are skipped via a `gate-args` step (`--skip env` when no `VERCEL_TOKEN`/link, `--skip push,digest` when no `CRON_SECRET`); `vercel link --yes` runs non-fatally first so check:env has a project link. Secrets to unlock the full gate: `VERCEL_TOKEN`, `CRON_SECRET`, `NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`. Docs: LAUNCH_CHECKLIST (Gate section), DEPLOYMENT (new CI gate section). YAML parses; no-secret CI scenario verified locally (`--skip env --skip push,digest` → gate exit 0).

## 2026-08-15 — Smoke test folds both cron dry checks (8 probes)
- **`scripts/launch-smoke-test.mjs`**: the JSON health report now carries a `cron` section — both deployed dry routes verified when `CRON_SECRET` is set: `/api/push/send?dry=1` (push_subscriptions, notifications, user_stats, get_leaderboard) **and** `/api/cron/digest?dry=1` (notifications, parent_digests, user_stats, get_leaderboard). New `summary.cronProbes` (8 total/ok/missing) gates the exit code the same way push did (unverified → non-gating). Removed the old top-level `push` key — it lives under `cron.push` now (single health artifact). Human output gained a `Digest:` line + `Cron probes: n/8 ok`. Live run: 0/8 missing (schema not applied), site 200, base 17/17. Syntax + lint clean, 195/195 tests.
